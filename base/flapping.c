...
void enable_flap_detection_routines(void) {
	...
	for(i = 0; i < num_objects.hosts; i++)
		check_for_host_flapping(&host_table[i], FALSE, FALSE, TRUE);
	for(i = 0; i < num_objects.services; i++)
		check_for_service_flapping(&service_table[i], FALSE, TRUE);
...
void disable_flap_detection_routines(void) {
	...
	/* handle the details... */
	for(i = 0; i < num_objects.hosts; i++)
		handle_host_flap_detection_disabled(&host_table[i]);
	for(i = 0; i < num_objects.services; i++)
		handle_service_flap_detection_disabled(&service_table[i]);
